...
# Reading in Twitter
file2 <- ('./Coursera-SwiftKey/final/en_US/en_US.twitter.txt')
con2 <- file(description=file2, "rb")
line3 <- readLines(con2, n=-1, warn=TRUE, skipNul = TRUE, encoding="UTF-8")
close(con2)
```
NEWS - total Length 1010242
1st Chunk: 1: 202048

TWITTER - total length 2360148
...
```{r}
#class(line2)
news2 <- as.data.frame(line2)
twitter2 <- as.data.frame(line3)


```

```{r}
#both <- rbind(news2, twitter2)
#colnames(both)<- "text"
#head(both)
both$text <- as.character(both$text)
```

```{r}
rm(list=setdiff(ls(), c("line2")))

### add a line to remove all variables except line2
...
myCorpus <- corpus(line2) # news originally
#docvars(myCorpus, "Source") <- "news"
#summary(myCorpus)
...
#SECTION TO TOKENIZE, THEN CREATE N-GRAMS
...
#saveRDS(trigram_split_df, file="triblogsamp_all_1.rds", ascii=FALSE)
saveRDS(blog3grams_ul, file="triblogsamp_news_all.rds", ascii=FALSE)
...
saveRDS(fivegram_split_df, file="fiveblogsamp_all_1.rds", ascii=FALSE)
...
saveRDS(fourgram_split_df, file="fourblogsamp_all_1.rds", ascii=FALSE)
...
saveRDS(bigram_split_df, file="biblogsamp_all_1.rds", ascii=FALSE)
...
saveRDS(unigram_count_dt, file="uniblogsamp_all_1.rds", ascii=FALSE)
